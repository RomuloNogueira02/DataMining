...
#           2020.03.31 a pyeclat function added for compatibility 
#                      with Sebastian Rashka's mlextend (afalcao)
#-----------------------------------------------------------------------
...
def pyeclat(trs, supp, target="s"):
    ...
    #target must be a string of: s, a, m or c for "all (s or a) maximal(m) or closed(c)
    target=target.lower()[0]
    if target not in ["s", "a", "m", "c"]: return None 
    supp=supp*100
    transactions=[frozenset(tr) for tr in trs]
    res = eclat(transactions, supp=supp, out=[], target=target)
    itemsets=[]
    supports=[]
    N=len(transactions)
    for i_s, n in res:
